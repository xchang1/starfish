...
class OrgJsonCommand:
    ...
    def add_to_parser(cls, subparser_root):
        ...
        org_json_command.add_argument(
            "--pretty", action="store_true", help="Pretty-print the output file")
    ...
    def run_command(cls, args, print_help=False):
        ...
        for new_key, old_key in (
            ...
            srcpath = os.path.join(input_dir, org_json_entry['file'])
            tile.set_source_fh_contextmanager(
                lambda path=srcpath: open(path, "rb"),
                ImageFormat.TIFF,
            )
            tile._name_or_url = org_json_entry['file']
            tileset.add_tile(tile)

            Writer.write_to_path(
                tileset,
                os.path.join(input_dir, f"{new_key}.json"),
                pretty=args.pretty,
                tile_opener=identity_file_namer,
                tile_writer=null_writer,
            )

            experiment['auxiliary_images'][new_key] = f"{new_key}.json"
